...
vector<Point> kmeans(vector<Point> &pts){
    vector<vector<Point>> centroids(1);
    for(int i = 0; i < K; i++){
        auto pt = Point(vector<double>{255.0/(K - 1) * (i), 255.0/(K - 1) * (i), 255.0/(K - 1) * (i)});
        centroids[0].push_back(pt);
    }
    while(true){
...
void part2(){
    ...
    auto centroids = kmeans(colors);

    for(auto &c : centroids){
        cout << (int) round(c.features[0]) << " " << (int) round(c.features[1]) << " " << (int) round(c.features[2]) << " ";
    }

    ofstream ofs("output.ppm");    
    ofs << "P3" << endl << x_dim << ' ' << y_dim << endl << "255" << endl;
    for(auto &c : colors){
        c = centroids[find_closest(centroids, c)];
        ofs << (int) round(c.features[0]) << " " << (int) round(c.features[1]) << " " << (int) round(c.features[2]) << " ";
    }
}
